                ...
                return getMdFiles(options.levels)
                    ...
                    .then(function (examples) {
                        ...
                        examples = examples.filter(function (example) {
                            return buildConfig.needNode(example.path);
                        });

                        return vow.all(examples.map(function (example) {
                            var dirname = path.join(root, example.path);
                            var bemjsonFilename = path.join(dirname, example.name + '.bemjson.js');
                            ...
                            return vfs.makeDir(dirname)
                                ...
                                .then(function () {
                                    buildConfig.addNode(example.path);
                    ...
                    .then(function (examples) {
                        ...
                        taskConfigurator.emit('inline-examples', options.destPath, examples);
                    });
            });
        }
    };
