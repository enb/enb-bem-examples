    ...
    return {
        ...
        configure: function (options) {
            ...
            helper.prebuild(function (magic) {
                var logger = config.getTaskConfig(helper.getTaskName()).getMakePlatform().getLogger();
                ...
                return getMdFiles(options.levels)
                    ...
                    .then(function (examples) {
                        ...
                        examples.forEach(function (example) {
                            example && (nodes[example.path] = true);
                        });

                        // Провайди полученные `bemjson.js` файлы
                        config.nodes(Object.keys(nodes), function (nodeConfig) {
